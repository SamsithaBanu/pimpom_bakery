...
import { UploadRecipeStyled } from "../../UploadRecipes/UploadRecipeStyled"; // Import the styled component
...
const AdminEditRecipe = ({ onClose, recipeData, fetchData }) => {
    const [formData, setFormData] = useState({
        _id: recipeData?._id || "", // Include _id here
        ...
        instructions: recipeData?.instructions?.length
          ...
          : [""],
        images: recipeData?.images?.length ? recipeData.images : [],
        equipments: recipeData?.equipments?.length
          ? recipeData.equipments
          : [""],
      });
      ...
      useEffect(() => {
        if (recipeData) {
          setFormData({
            _id: recipeData._id || "", // Ensure _id is included when recipeData changes
            name: recipeData.name || "",
            description: recipeData.description || "",
            servings: recipeData.servings || 4,
            prepTime: recipeData.prepTime || "",
  ...
  console.log('fotm corre', formData);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(SummaryApi.updateRecipe.url, {
      method: SummaryApi.updateRecipe.method,
      credentials: "include",
      headers: {
        "content-type": "application/json",
      },
      body: JSON.stringify(formData), // Ensure the _id is sent along with other data
    });
      ...
      <div className="uploadRecipeWrapper">
        <div className="recipeWrapper">
          <div className="headerWrapper">
            <h2 className="heading">Update Recipe</h2>
